...
fn main() {
    ...
    for arg in args {
        ...
        //
        if arg=="sum"{ // - If arg is "sum", then call the sum() function
            sum();
        }else if arg=="double"{  // - If arg is "double", then call the double() function
            double();
        }else{
            count(arg); // - If arg is anything else, then call the count() function, passing "arg" to it.
        }
